...
if(isset($_POST['message_customer'])){

    $user_id = $_POST['message_id'];
    $order_number = $_POST['transaction_input'];
    ...
    $total_price = 0;
    //order information
    $order_query = mysqli_query($conn, "SELECT * FROM `transaction` WHERE order_number = '$order_number'") or die("query failed");
    while($fetch_order = mysqli_fetch_assoc($order_query)) {

        $item_id = $fetch_order['item_id'];
        $select_plant = mysqli_query($conn, "SELECT * FROM `product` WHERE item_id = '$item_id'") or die("query failed");
        while($fetch_plant = mysqli_fetch_assoc($select_plant)){
            $item_brand = $fetch_plant['item_brand'];
        }
        ...
        $cancel_reason = $fetch_order['cancel_reason'];
        $size = "Size: ";
    ...
    $template_file = "./template_transaction.php";
    ...
    if($item_status === "Cancelled"){
        //Location of template file
        $template_file = "./template_transaction_cancelled.php";
        ...
        echo 'alert("Send Transaction Message Successfully")';  //not showing an alert box.
        echo '</script>';
        echo "<script>window.top.location='./transaction'</script>";
    }else{
...
if(isset($_POST['delete_transaction'])){
    $order_number = $_POST['transaction_input'];
    mysqli_query($conn, "DELETE FROM `transaction` WHERE order_number ='$order_number'") or die("query failed");
    echo '<script type="text/javascript">';
    echo 'alert("Delete Transaction Successfully")';  //not showing an alert box.
    echo '</script>';
    echo "<script>window.top.location='./transaction'</script>";
}
...
<div id="content-wrapper" class="d-flex flex-column">
    ...
    <div id="content">
        ...
        <div class="container py-1" style="position: relative;">
            <h4 class="font-eduvic font-size-24" style="color: black;">Transaction</h4>
            <div class="filters">
            Filter by: <select name="fetchtransaction" id="fetchtransaction" >
            ...
            <?php
                ...
                echo "Filter by Date: <select name='fetchtransactionmonth' id='fetchtransactionmonth'>";
                ...
                echo "<select name='fetchtransactionday' id='fetchtransactionday' disabled>";
        ...
        <div class="containers py-1" style="width: 1300px; height: 550px;overflow: auto;">
            ...
            <div class="container py-1 my-1 col-auto">
                <div class="row  font-rale">
                    ...
                    if(isset($search)){
                    $checkout_query = mysqli_query($conn, "SELECT DISTINCT order_number, user_id, item_status FROM `transaction`  WHERE order_number LIKE '%$search%' AND (item_status = 'Completed' or item_status = 'Cancelled') or mode_of_payment LIKE '%$search%' AND (item_status = 'Completed' or item_status = 'Cancelled')  or customer_name LIKE '%$search%' AND (item_status = 'Completed' or item_status = 'Cancelled') or item_status LIKE '%$search%' ORDER BY order_number") or die('query failed');
                    $grand_total = 0;
                    if(mysqli_num_rows($checkout_query) > 0){
                        ...
                        ?>
                            ...
                            <h6>Reason for Cancellation: <?php echo $cancel_reason; ?> </h6>
                            ...
                            <div class="container" style="text-align: center; padding: 0px; padding-bottom:20px; padding-top:10px;">
                                    <form action="" method="post">
                                        <input type="hidden" name="transaction_input" value="<?php echo $fetch_checkout['order_number'] ?? '000001' ?>">
                                        <input type="hidden" name="message_id" value="<?php echo$fetch_checkout['user_id'] ?? 0; ?>">
                                        <button type="submit"  name="message_customer" class="btn btn-success" >Send Invoice</button>
                        ...
                        echo '<tr><td style="padding:20px; text-transform:capitalize;" colspan="6">Empty Transaction</td></tr>';
                    }
                    ?>
                    <?php
                    }else{
                        $checkout_query = mysqli_query($conn, "SELECT DISTINCT order_number, user_id, item_status FROM `transaction` WHERE (item_status = 'Completed' or item_status = 'Cancelled') ORDER BY order_number") or die('query failed');
                        $grand_total = 0;
                        if(mysqli_num_rows($checkout_query) > 0){
                            while($fetch_checkout = mysqli_fetch_assoc($checkout_query)){
                                $order_number = $fetch_checkout['order_number'];
