        ...
        <div class="container py-1 my-1 col-auto">
            ...
            <?php
                    ...
                    if(isset($search)){
                        ...
                        <div class="order-body">
                            ...
                            <div class="container w-75 " style="text-align: center; padding: 20px; margin-left:30px; position: absolute; bottom: 0px; ">
                                <form action="message-supplier.php" method="post">
                    ...
                    <?php
                        ...
                        if($request === 'Username'){
                            $checkout_query = mysqli_query($conn, "SELECT user_id FROM `supplier` ORDER BY username") or die('query failed');
                        }elseif($request === 'ID'){
                            ...
                            $checkout_query = mysqli_query($conn, "SELECT user_id FROM `supplier` ORDER BY first_name") or die('query failed');
                        }elseif($request === 'Last Name'){
                            $checkout_query = mysqli_query($conn, "SELECT user_id FROM `supplier` ORDER BY last_name") or die('query failed');
                        }elseif($request === 'A-Z'){
                            $checkout_query = mysqli_query($conn, "SELECT user_id FROM `supplier` ORDER BY first_name") or die('query failed');
                        }elseif($request === 'Z-A'){
                            $checkout_query = mysqli_query($conn, "SELECT user_id FROM `supplier` ORDER BY first_name DESC") or die('query failed');
                        }else{
