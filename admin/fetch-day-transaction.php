...
?>
        ...
        <?php
                    ...
                    <?php
                        ...
                        $checkout_query = mysqli_query($conn, "SELECT DISTINCT order_number, user_id, item_status FROM `transaction` WHERE (item_status = 'Completed' or item_status = 'Cancelled') AND date_delivery = '$month' AND date_delivery_day = '$request' ORDER BY order_number") or die('query failed');
                        $grand_total = 0;
                        if(mysqli_num_rows($checkout_query) > 0){
                            while($fetch_checkout = mysqli_fetch_assoc($checkout_query)){
                                $order_number = $fetch_checkout['order_number'];
