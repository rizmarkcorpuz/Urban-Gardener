...
if(isset($_POST['reply_review'])){
    ...
    $reply_query = mysqli_query($conn, "INSERT INTO `review_table` (parent_id, item_id, item_name, user_name, first_name, last_name, user_image, user_rating, user_review, datetime) VALUES ('$review_id', '$item_id', '$item_name', 'urbangardenercavite', 'Urban', 'Gardener', '$user_image', 0, '$user_review', '$time')") or die("query failed " . mysqli_error($conn));
    echo '<script type="text/javascript">';
    ...
    <div id="content">
        ...
        <div class="container py-1 my-1 ">
                ...
                if(isset($search)){
                    ...
                    $review_all_query = mysqli_query($conn, "SELECT DISTINCT item_id FROM `review_table` WHERE parent_id = '0' AND user_name LIKE '%$search%' or parent_id = '0' AND first_name LIKE '%$search%' or parent_id = '0' AND last_name LIKE '%$search%' or parent_id = '0' AND user_rating LIKE '%$search%' or parent_id = '0' AND user_review LIKE '%$search%'  ORDER BY review_id DESC") or die('query failed');
                            $reviewHTML = '';
                            $total_rating = 0;
                            $average_rating = 0;
                            $total_review = 0;
                            ...
                            if(mysqli_num_rows($review_all_query) > 0){
                                ...
                                while($fetch_review = mysqli_fetch_assoc($review_all_query)){
                                    ...
                                    while($fetch_review = mysqli_fetch_assoc($review_query)){
                                        ...
                                        $username = $fetch_review['user_name'];
                            ...
                            // item id need to declare
                            $review_query = mysqli_query($conn, "SELECT * FROM `review_table`  WHERE item_id = '$item_id' AND parent_id = '0' AND user_name LIKE '%$search%' or item_id = '$item_id' AND parent_id = '0' AND first_name LIKE '%$search%' or item_id = '$item_id' AND parent_id = '0' AND first_name LIKE '%$search%' or item_id = '$item_id' AND parent_id = '0' AND last_name LIKE '%$search%' or item_id = '$item_id' AND parent_id = '0' AND user_rating LIKE '%$search%' or item_id = '$item_id' AND parent_id = '0' AND user_review LIKE '%$search%' ORDER BY review_id DESC") or die("query failed");
                            $total_rating = 0;
                            $average_rating = 0;
                            $total_review = 0;
                            $rounded_rating = 0;
                            ...
                            while($fetch_review = mysqli_fetch_assoc($review_query)){
                                ...
                                $i = 0;
                                    ...
                                    $reviewHTML .='
                                        <div class="dropdown" style="float:right;">
                                            ...
                                            <ul class="menu border">
                                                ...
                                                <button type="button" class="no-style-button" data-toggle="modal" data-target="#deleteReview' . $id_review .'">
                                                Delete
                                                </button>
                                                <button type="button" class="no-style-button reply" onclick="getReviewId(';
                                                $reviewHTML .= $item_id . ", " . $id_review . ", '" . $full_name . "', '" . $user_review . "', '" . $user_rating . "'";
                                                $reviewHTML .=')" data-toggle="modal" data-target="#replyReview' . $item_id .'">Reply</button>       
                                            </ul>
                                        </div>';
                                    ...
                                    $reviewHTML .='
                                        ...
                                        <!-- Delete Modal -->
                                        <div class="modal fade" id="deleteReview' . $id_review . '" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                        ...
                                        <!-- Reply Modal -->
                                        <div class="modal fade" id="replyReview' . $item_id . '" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                            <div class="modal-header">
                                            ...
                                            $reviewHTML .= '
                                                <div class="row" style="position:relative;">
                                                    ...
                                                    <div class="col panel panel-primary">
                                                        <div id="showStar' . $item_id . '"></div>
                                                        <h6>By <b style="color:black;" id="fullName' . $item_id . '">' . $full_name . '</b></h6>
                                                        <h6 style="color:black;" id="userReview' . $item_id . '"></h6>
                                                        <input type="hidden" id="userRating' . $item_id . '"  value="' . $user_rating .' ">
                                                    </div>
                                                </div>
                                            </div>
                                            <form action="" method="post">
                                                ...
                                                <div class="modal-footer">
                                                    <input type="hidden" id="reviewId" name="review_id" value="' . $id_review .' ">
                                                    <input type="hidden" id="itemId" name="item_id" value="' . $item_id .' ">
                                                    <input type="hidden" id="itemName" name="item_name" value="' . $item_name .' ">
                                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                    ...
                    //echo $reviewPlantHTML;
                            ...
                            $review_all_query = mysqli_query($conn, "SELECT DISTINCT item_id FROM `review_table` ORDER BY review_id DESC") or die('query failed');
                            $reviewHTML = '';
                            $total_rating = 0;
                            $average_rating = 0;
                            $total_review = 0;
                                            ...
                                            <div class="modal fade" id="deleteReview' . $id_review . '" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                <div class="modal-dialog" role="document">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-footer">
                                                            <form action="" method="post">
                                                                <input type="text" name="review_id" value="' . $id_review .' ">
                                                                <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                                                <button type="submit" name="delete_review" class="btn btn-danger">Delete</button>
                                                            </form>
                                                        </div>
...
function getReviewId(item_id, review_id, name, review, rating){

    //var review_name = review_id;
    //var name = full_name;
    //var review = user_review;
    var rating_data = rating;

    //document.getElementById('#reviewId');

    $('#reviewId').val(review_id);
    $('#fullName').html(name);
    $("#userReview" + item_id).html(review);
    $("#userRating" + item_id).val(rating);
    ...
    data: {rating_data:rating_data},
        success:function(response) {
            //alert(rating_data);
            $('#showStar' + item_id).html(response);
        }
    })
}
