...
if(isset($_POST['payment'])){
    ...
    $delivery_day = $_POST['date_delivery_day'];
    ...
    $selectall_cart = mysqli_query($conn, "INSERT INTO `checkout` (user_id, customer_name, item_id, item_brand, item_name, item_price, item_image, item_quantity, item_size, pot, order_number, mode_of_payment, date_delivery, date_delivery_day) SELECT user_id, '$customer_name', item_id, item_brand, item_name, item_price, item_image, item_quantity, item_size, pot, '$order_number', '$mode_of_payment', '$delivery_date', '$delivery_day' FROM `cart` WHERE user_id = '$user_id'") or die("query failed");
    $notification_query = mysqli_query($conn, "INSERT INTO `notifications` (text, date, url, status) VALUES ('You have an order to confirm', Now(), 'orders.php', '0')") or die("query failed");
    $quantity_query = mysqli_query($conn, "SELECT * FROM cart WHERE user_id='$user_id'") or die("query failed");
    while($fetch_quantity = mysqli_fetch_assoc($quantity_query)){
        $item_id = $fetch_quantity['item_id'];
        $item_size = $fetch_quantity['item_size'];
        ...
        while ($get_quantity = mysqli_fetch_assoc($product_query)){
            ...
            if($item_size == "small"){
                $product_quantity = $get_quantity['small_quantity'];
                $product_quantity = $product_quantity - $item_quantity;
                $update_query = mysqli_query($conn, "UPDATE `product` SET small_quantity='$product_quantity' WHERE item_id='$product_id'");
            }elseif($item_size == "medium"){
                $product_quantity = $get_quantity['medium_quantity'];
                $product_quantity = $product_quantity - $item_quantity;
                $update_query = mysqli_query($conn, "UPDATE `product` SET medium_quantity='$product_quantity' WHERE item_id='$product_id'");
            }elseif ($item_size == "large"){
                $product_quantity = $get_quantity['large_quantity'];
                $product_quantity = $product_quantity - $item_quantity;
                $update_query = mysqli_query($conn, "UPDATE `product` SET large_quantity='$product_quantity' WHERE item_id='$product_id'");
            }
    ...
    echo '<script type="text/javascript">';
    echo 'alert("Check out Successfully")';  //not showing an alert box.
    echo '</script>';
...
<section id="cart" class="py-3 mb-5">
    ...
    <div class="container-fluid w-75" style="margin-top: 100px">
        <h5 class="font-baloo font-size-24" style="margin-bottom:-10px;"><b>Checkout</b></h5>
            <div class="row">
                <div class="col-sm-8">
                    <?php
                    if(isset($_POST['user_id'])){
                            ...
                            while($fetch_cart = mysqli_fetch_assoc($cart_query)){
                                ...
                                <div class="row border-top py-3 mt-3">
                                    ...
                                    <div class="col-sm-8">
                                        ...
                                        <small>Quantity: <?php echo $fetch_cart['item_quantity'] ?? "Brand"; ?></small><br>
                                        <?php 
                                    ...
                                    <div class="col-sm-1 text-right">
                                        <div class="font-size-20 text-danger font-baloo">
                                            ...
                                            ₱<span class="product_price" data-id="<?php echo $fetch_cart['item_id'] ?? 0; ?>"><?php if($sub_total == 0){
                                                    echo 0;
                                                }else{
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        <form action="" method="post" class="modal-body">
                                            ...
                                            <button class="btn btn-primary mt-3" type="submit" name="payment">Confirm</button>
                                        </form>
