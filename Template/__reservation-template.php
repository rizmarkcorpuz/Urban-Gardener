        ...
        if($_POST['user_id'] == 0){
            ...
            if($row['email_verified'] == NULL){
                ...
                if (isset($_POST['top_sale_submit'])) {
                    ...
                    $item_quantity = $_POST['input_quantit'];
...
if(isset($_POST['reserve'])){
    ...
    header("Location: reservation-checkout");
...
if(isset($_POST['update-quantity'])){
    ...
    $select_cart = mysqli_query($conn, "SELECT * FROM `reservation` WHERE user_id = '$user_id'") or die('query failed');
    ...
    if(mysqli_num_rows($select_cart) > 0 ) {
        ...
        //mysqli_query($conn, "DELETE FROM `cart` WHERE cart_id = '$cart_id'");
        $quantity_query = mysqli_query($conn, "SELECT item_quantity FROM `product` WHERE item_id ='$item_id'") or die("query failed");
        $fetch_quantity = mysqli_fetch_assoc($quantity_query);
        $product_quantity = $fetch_quantity['item_quantity'];
        $sum = $product_quantity - $item_quantity;
        if($item_quantity >10){
            $message[] = 'Item quantity cannot exceed 10';
            mysqli_query($conn, "UPDATE `reservation` SET item_quantity = '10' WHERE user_id = '$user_id' AND item_id = '$item_id'") or die('query failed');
        }elseif($item_quantity <=0){
            mysqli_query($conn, "UPDATE `reservation` SET item_quantity = '1' WHERE user_id = '$user_id' AND item_id = '$item_id'") or die('query failed');
        }else{
            mysqli_query($conn, "UPDATE `reservation` SET item_quantity = '$item_quantity' WHERE user_id = '$user_id' AND item_id = '$item_id'") or die('query failed');
        }
...
//delete cart item
if (isset($_POST['delete-reservation-submit'])){
    $delete_id = $_POST['reservation_id'];
    $delete_query = mysqli_query($conn, "DELETE FROM reservation WHERE reservation_id='$delete_id'") or die("query failed");
}
...
<section id="cart" class="py-3 mb-5">
    <div class="container-fluid w-75" style="margin-top: 100px;">
        <h5 class="font-baloo font-size-20">Reservation Cart</h5>
                                    ...
                                    <div class="qty d-flex pt-2">
                                        <div class="d-flex font-rale w-25">
                                            ...
                                            <form action="" method="post" enctype="multipart/form-data">
                                                <input type="hidden" name="item_id" value="<?php echo $fetch_reservation['item_id'] ?? '0'; ?>">
                                                <input type="number" id="input_quantity" name="input_quantity"  class="qty_input border px-2 w-50 h-10 bg-light text-center p-2 m-2" min="1" value="<?php echo $fetch_reservation['item_quantity'] ?? 1 ?>" placeholder="1">
                                        ...
                                        <form method="post">
                                            <input type="hidden" value="<?php echo $fetch_reservation['reservation_id'] ?? 0; ?>" name="reservation_id">
                                            <button type="submit" name="delete-reservation-submit" class="btn btn-danger font-size-12 px-2">Delete</button>
                                        </form>
                ...
                <div class="sub-total border text-center mt-2">
                    ...
                    <div class="border-top py-4">
                        ...
                        <form action="" method="post">
                            <input type="hidden" value="<?php echo $fetch_reservation['cart_id'] ?? 0; ?>" name="cart_id">
                            <input type="hidden" name="item_id" value="<?php echo $fetch_reservation['item_id'] ?? '0'; ?>">
                            <input type="hidden" name="item_brand" value="<?php echo $fetch_reservation['item_brand'] ?? "Unknown" ?>">
                            <input type="hidden" name="item_name" value="<?php echo $fetch_reservation['item_name'] ?? "Unknown" ?>">
                            <input type="hidden" name="item_price" value="<?php echo $fetch_reservation['item_price'] ?? "Unknown" ?>">
                            <input type="hidden" name="item_quantity" value="<?php echo $fetch_reservation['item_quantity'] ?? "1" ?>">
                            <input type="hidden" name="item_image" value="<?php echo $fetch_reservation['item_image'] ??  "Unknown" ?>">
                            <input type="hidden" name="user_id" value="<?php echo 1; ?>">
                            <?php
                            if($grand_total == 0){
                                echo '<button type="submit" name="reserve" class="btn btn-danger mt-3" disabled>No Items to Reserve</button>';
                            }
                            else{
                                echo '<button type="submit" name="reserve" class="btn btn-warning mt-3">Proceed to Reserve</button>';
                            }
                            ?>
    ...
    if(isset($_GET['item_id_popup'])) {
        ...
        <form action="" method="post">
            ...
            <div class="d-block">
                ...
                <input type="hidden" name="input_quantit" id="input_quantit" value="1">
